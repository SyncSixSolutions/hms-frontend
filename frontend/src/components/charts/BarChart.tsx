    ...
    <div className="w-full">
      {title && (
        ...
        <div className="flex items-end justify-around h-full">
          {data.map((item, index) => (
            <div key={index} className="flex flex-col items-center">
              <div
                className={`rounded-t-md transition-all hover:opacity-80 w-[${barWidth}px] bg-[${item.color || defaultColor}]`}
                style={{
        ...
        <div className="absolute top-0 left-0 h-full w-full -z-10">
          {[0.25, 0.5, 0.75, 1].map((tick, i) => (            <div
              key={i}
              className={`absolute border-t border-gray-200 w-full top-[${height - (tick * (height - 40) + 40)}px]`}
            >
